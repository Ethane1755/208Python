...
pointer = arrow(shaftwidth=0.01)
pointer1 = arrow(shaftwidth=0.01,color = color.green)
pointer2 = arrow(shaftwidth=0.01,color = color.red)

c = curve([vector(d*i*0.9, d*i*0.1, 0) for i in range(N)])
...
while True:
    ...
    if T + dt >50*dt and T < 50*dt:  #在T被歸零前的那瞬間更新畫面圖形       #計時器
        for i in range(N):
        ...
        pointer.pos = vector(ball_pos[0][0],ball_pos[1][0],0)
        pointer.axis = vector(-spring_axis[0][0]*10,spring_axis[1][0]*10,0)
        pointer1.pos = vector(ball_pos[-1][0],ball_pos[-1][1],0)
        pointer1.axis = vector(spring_axis[-1][0]*10,spring_axis[-1][1]*10,0)
        ball_cm = np.sum(ball_pos, axis = 0)/N  #這是個1維3個元素的array
        pointer2.pos = vector(ball_cm[0],ball_cm[1],0)
        pointer2.axis = vector(0,-m*N,0)
