...
scene = canvas(title='Spring Wave', width=200, height=1200, range = 0.4*50/6) 
ball = [sphere(radius=size, color=color.yellow, pos=vector(0, -i*d, 0), v=vector(0,0,0)) for i in range(N)]
#以list comprehensions方式產生N個球
spring = [helix(radius = size/2.0, thickness = d/15.0, pos=vector(0, i*d, 0), axis=vector(0,d,0)) for i in range(N-1)]
...
while True:
    rate(1000)
    t += dt
    ball[0].pos = vector(A*cos(omega*t), 0, A*sin(omega*t))    ##強迫第一個球沿x方向振動
    ball[-1].pos = ball[-1].pos
    #在球與球之間放入彈簧
