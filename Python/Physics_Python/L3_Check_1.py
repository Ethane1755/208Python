...
v_vector_1 = arrow(color=color.red)  #畫描述v的箭頭
v_text_1 = label(box = False, opacity = 0, height = 25, color=color.red, text = 'a')  #產生文字標籤'v'
...
while True:
    ...
    v_vector_1.pos = ball.pos    #將速度箭頭的位置放在球的位置
    if mag(ball.v) > 0: v_vector_1.axis = ball.a/mag(ball.a)*R/2    #將速度箭頭的軸方向指向速度方向，並將箭頭的長度設定為半徑的一半
    
    v_text.pos = v_vector.pos + v_vector.axis*1.2 #文字標籤'v'的位置
    v_text_1.pos = v_vector.pos + v_vector_1.axis*1.2 #文字標籤'v'的位置
    
    print('simulated acceleration:', mag(ball.a), ' acceleration:',ball_a_r) 